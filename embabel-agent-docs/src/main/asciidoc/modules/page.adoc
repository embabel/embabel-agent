...
| `embabel-agent-decision`
| This repo
| Typed decision contracts, validation, records, `NoDecisionModel`, and `StubDecisionModel`
| Framework-neutral decision facade; registered instances are the third `ModelProvider` model type
| Experimental

| `embabel-agent-decision-typesafe`
| This repo
| Jev distribution-backed decisions
| Depends directly on `embabel-agent-decision` and `embabel-agent-ai`
| Experimental

| `embabel-agent-decision-llm`
| This repo
| Prompted verbalized decisions
| Depends on `embabel-agent-decision` and `embabel-agent-api`
| Experimental

| `embabel-agent-decision-autoconfigure`
| This repo
| Opt-in Spring Boot configuration for typed decisions
| Registers one or more named decision models; an empty enabled declaration supplies Jev
| Experimental
...
| `embabel-agent-starter-decision`
| This repo
| Opt-in typed decision starter
| Adds the platform and decision autoconfiguration; configuration remains disabled by default
| Experimental
|===
