...
MatryoshkaTool requires the `MatryoshkaToolInjectionStrategy` to be enabled.
This strategy watches for MatryoshkaTool invocations and performs the tool replacement.

When using Embabel's own tool loop (the default), enable it via the injection strategy:
...
[source,java]
----
import com.embabel.agent.spi.loop.MatryoshkaToolInjectionStrategy;
import com.embabel.agent.spi.loop.CompositeToolInjectionStrategy;

// Use the singleton instance
MatryoshkaToolInjectionStrategy strategy = MatryoshkaToolInjectionStrategy.INSTANCE;

// Or combine with other strategies
CompositeToolInjectionStrategy combined =
    CompositeToolInjectionStrategy.withMatryoshka(otherStrategy1, otherStrategy2);
----
...
// Use the singleton instance
val strategy = MatryoshkaToolInjectionStrategy.INSTANCE

// Or combine with other strategies
val combined = CompositeToolInjectionStrategy.withMatryoshka(otherStrategy1, otherStrategy2)
----
====
