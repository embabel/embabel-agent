...
SimpleAgenticTool withDynamicPrompt = base.withSystemPromptCreator(agentProcess -> {
    String userId = agentProcess.getBlackboard().get("userId", String.class);
    return "Process requests for user " + userId + " using the available tools";
});
----
...
It creates Embabel `Tool` instances from MCP servers, with support for filtering tools and wrapping them in `MatryoshkaTool` facades.
...
TIP: MCP clients are configured in `application.yml`. See <<reference.integrations__mcp,MCP Integration>> for configuration details.
...
===== Creating MatryoshkaTool Facades from MCP

`McpToolFactory` can wrap groups of MCP tools in a `MatryoshkaTool` facade for progressive disclosure.
...
[source,java]
----
// Create a MatryoshkaTool with specific tool names
MatryoshkaTool wikipediaTool = mcpToolFactory.matryoshkaByName(
    "wikipedia",
...
MatryoshkaTool dbTool = mcpToolFactory.matryoshkaMatching(
    "database_operations",
...
MatryoshkaTool webTool = mcpToolFactory.matryoshka(
    "web_operations",
...
By default, `MatryoshkaTool` facades created by `McpToolFactory` are removed after invocation (replaced by inner tools).
...
MatryoshkaTool persistentTool = mcpToolFactory.matryoshkaByName(
    "wikipedia",
    "Search Wikipedia",
...
public class ChatConfiguration {
    ...
    public CommonTools commonTools(McpToolFactory mcpToolFactory) {
        ...
        // MatryoshkaTool grouping related Wikipedia MCP tools
...
* **Groups related tools** behind a descriptive facade using `MatryoshkaTool`
...
|Create a `MatryoshkaTool` containing tools with exact matching names.

|`matryoshkaMatching(name, description, patterns, removeOnInvoke?)`
|Create a `MatryoshkaTool` containing tools matching any of the regex patterns.

|`matryoshka(name, description, filter, removeOnInvoke?)`
|Create a `MatryoshkaTool` with a custom filter predicate.
|===
