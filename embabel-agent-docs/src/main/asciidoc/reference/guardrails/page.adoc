...
When validating user input from multiple messages, the _UserInputGuardRail_ interface provides a _combineMessages_ method that can be overridden to customize how messages are combined before validation.

The default implementation joins all user message contents with newlines:
...
Override this method to customize message combining for your guardrail:

[source,kotlin]
----
class CustomCombineGuardRail : UserInputGuardRail {
    override val name = "CustomCombineGuard"
    override val description = "Guardrail with custom message combining"

    override fun combineMessages(userMessages: List<UserMessage>): String {
        // Custom: add context markers between messages
    ...
    override fun validate(input: String, blackboard: Blackboard): ValidationResult {
        // input now contains the custom-formatted combined messages
        return ValidationResult.VALID
    }
}
----

This allows guardrails to:

- Add separators or context markers between messages
- Filter out certain message types
- Apply message-specific preprocessing
- Implement conversation-aware validation logic
