...
|`ToolNamingStrategy`
|`LEGACY`
|`LEGACY` keeps the current tool names. `FULL_HIERARCHY` prefixes names with the stable owner hierarchy to avoid collisions between tools from different consumers or agents.
