...
With `FULL_HIERARCHY`, tools owned by a `ToolConsumer` are prefixed with that consumer's full hierarchy name,
while generated goal tools without an explicit export name are prefixed with their owning agent's name.
Framework-owned HITL callback tools retain their reserved names.
For example, `LEGACY_NAME_ONLY` publishes `search` as `search`; with `FULL_HIERARCHY`, a consumer whose hierarchy is
`AgentA.run` publishes it as `AgentA_run_search`, and a nested hierarchy `Outer.Inner` publishes it as
`Outer_Inner_search`.
...
|`ToolNamingStrategy`
|`LEGACY_NAME_ONLY`
|`LEGACY_NAME_ONLY` keeps only the current tool names. `FULL_HIERARCHY` prefixes names with the stable owner hierarchy to avoid collisions between tools from different consumers or agents.

|===

.Example - opt in to hierarchy-based tool names
[source,yaml]
----
embabel:
  agent:
    platform:
      tools:
        naming-strategy: full-hierarchy
----

The resulting tool names are sanitized exposed names, not full JVM signatures.
