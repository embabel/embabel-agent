...
Controls how tools are named when they are exposed to LLMs or generated tool endpoints.
The default preserves existing tool names for backwards compatibility.
With `FULLY_QUALIFIED`, every published tool is qualified by its owning agent. For example,
`LEGACY_NAME_ONLY` publishes `search` as `search`; `FULLY_QUALIFIED` publishes it under `AgentA` as `AgentA-search`.
Thus, `AgentA` and `AgentB` publish distinct names. Explicit export names remain unchanged. A goal exported for
several starting input types also carries the input type name, which falls back to the package qualified type when
two of them share a simple name. Framework-owned HITL callback tools published alongside goal tools retain their
reserved names; HITL tools reaching an LLM as ordinary interaction tools are qualified like any other.
...
|`ToolNamingStrategy`
|`LEGACY_NAME_ONLY`
|`LEGACY_NAME_ONLY` preserves current names. `FULLY_QUALIFIED` qualifies names with the owning agent.

|===

.Example - opt in to fully qualified tool names
[source,yaml]
----
embabel:
  agent:
    platform:
      tools:
        naming-strategy: fully-qualified
----

With `FULLY_QUALIFIED`, non-alphanumeric characters in the owner are encoded (`.` becomes `_2e_`)
before the owner and tool are joined with `-`. Generated tool names must match `[a-zA-Z0-9_-]+`, and
the complete published name is limited to 64 characters. Invalid or overlong generated names fail
with `IllegalArgumentException` rather than being rewritten or truncated.

An explicit `export.name` is not qualified or normalized. It is used as supplied for one input type;
for multiple input types, the input-type discriminator is appended to it. The resulting name is
length-checked and must fit within 64 characters.

Naming does not check uniqueness across the complete published tool list. Duplicate explicit names,
collisions with generated or platform tools, and input-type discriminator collisions may still be
dropped or rejected by a publication path. Full-list collision validation is tracked separately in
issue #1834.
