...
With `FULLY_QUALIFIED`, tools are qualified by their owner, the agent name and the action's short name, while generated goal tools
without an explicit export name are qualified by their owning agent. Framework-owned HITL callback tools published
alongside goal tools retain their reserved names; HITL tools reaching an LLM as ordinary interaction tools are
qualified like any other. For example, `LEGACY_NAME_ONLY` publishes `search` as `search`; `FULLY_QUALIFIED` publishes
it under `AgentA.run` as `AgentA_2e_run-search`. Thus, owners `AgentA.run` and `AgentB.run` publish distinct names.
Explicit export names remain unchanged.
...
|`ToolNamingStrategy`
|`LEGACY_NAME_ONLY`
|`LEGACY_NAME_ONLY` preserves current names. `FULLY_QUALIFIED` qualifies names with the owning agent and action.
