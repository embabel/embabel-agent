...
Non-alphanumeric characters in each part are encoded (`.` becomes `_2e_`); owner and tool are joined with `-`, which never appears inside an encoded part. Names longer than 64 characters use a stable hash suffix.
