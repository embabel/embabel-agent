...
With `FULLY_QUALIFIED`, tools are qualified by their complete Embabel owner name, while generated goal tools
without an explicit export name are qualified by their owning agent. Framework-owned HITL callback tools retain
their reserved names. For example, `LEGACY_NAME_ONLY` publishes `search` as `search`; `FULLY_QUALIFIED` publishes
it under `AgentA.run` as `AgentA_2e_run_search`. Thus, owners `AgentA.run` and `AgentB.run` publish distinct names.
...
|`ToolNamingStrategy`
|`LEGACY_NAME_ONLY`
|`LEGACY_NAME_ONLY` preserves current names. `FULLY_QUALIFIED` qualifies names with the complete Embabel owner.
...
Non-alphanumeric characters are encoded (`.` becomes `_2e_`). Names longer than 64 characters use a stable hash suffix.
