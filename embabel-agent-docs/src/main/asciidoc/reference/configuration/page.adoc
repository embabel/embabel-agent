...
Controls how goal tools exported through MCP are named. The default keeps existing names.
...
|`ToolNamingStrategy`
|`LEGACY_NAME_ONLY`
|`FULLY_QUALIFIED` prefixes each goal tool with its owning agent.
...
* An explicit `export.name` is used as is (`my.export`, or `my.export-UserInput` for several input types).
* Names longer than 64 characters fail at startup instead of being truncated.

Duplicate names are not yet detected; see issue #1834.
