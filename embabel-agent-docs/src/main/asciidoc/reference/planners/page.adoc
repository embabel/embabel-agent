...
Hybrid is the messy middle. It picks the highest-netValue achievable action each tick (so research happens) *and* checks "is the goal already satisfied?" **before** selecting an action (so termination is clean once a real goal is reached).
