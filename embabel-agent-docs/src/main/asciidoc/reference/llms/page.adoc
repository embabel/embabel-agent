...
A runner declares that by implementing `LateArrivingModels` alongside its resolver; without it, the same entry under the deployment's own provider is still a typo and still fatal.
...
To do the same for a runner the framework does not ship, implement `LocalModelSource` — what it is serving, with what each model is for, and how to build a service for one — and register a `LocalModelCatalog` bean over it, plus a `LocalModelRoleResolver` and `LocalModelEmbeddingRoleResolver` sharing that same catalog.
