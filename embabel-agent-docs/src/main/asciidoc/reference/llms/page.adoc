...
so the caller sees the same actionable "no LLM configured" error. A name that is neither a role nor
a registered model, with no placeholder to fall back to, is still fatal at startup.
