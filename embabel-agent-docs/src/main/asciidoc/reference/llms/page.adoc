...
- **`AutoCorrectionPolicy`** (default) — feeds the error back to the LLM so it can self-correct. Includes suffix-match suggestions for hallucinated prefixes (e.g., `ragbot_vectorSearch` → suggests `vectorSearch`). Throws `ToolNotFoundException` after 3 consecutive failures.
- **`ImmediateThrowPolicy`** — throws `ToolNotFoundException` immediately.

The system-wide default is configured via properties:
...
embabel:
  agent:
    platform:
      toolloop:
        tool-name-auto-correction: true  # false selects ImmediateThrowPolicy
----
...
[source,kotlin]
----
class MyFuzzyMatchPolicy : ToolNotFoundPolicy {
