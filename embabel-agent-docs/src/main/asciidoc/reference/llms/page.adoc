...
Startup is a separate question. An unsatisfiable role only logs a warning when the platform starts,
so a deployment keyed for one provider still boots and still serves every role that does work - you
find out about the broken role when something asks for it, and the message names the role, the
active provider, and the models that were available.
...
The context does not propagate to threads you spawn yourself. Capture it with
