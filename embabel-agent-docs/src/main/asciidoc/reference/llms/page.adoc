...
In Java, use static imports to keep additional `LlmOptions` configuration in the same chain:
...
- `NativeStructuredOutputMode.ENABLED`: Use the native path unconditionally when the provider supports it, bypassing the schema compatibility check.
- `NativeStructuredOutputMode.DEFAULT`: Check schema compatibility first; fall back to prompt-based extraction with a WARN log if the schema is not compatible. Use `DISABLED` to suppress the warning.
- `NativeStructuredOutputMode.DISABLED`: Always use prompt-based extraction; never use the native path.
