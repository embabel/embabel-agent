...
IMPORTANT: Be sure that the `AgentPlatform` has loaded all its actions before creating an `AgentProcessChatbot`.
Otherwise the actions needed to respond to chat may not be available to the chatbot.
