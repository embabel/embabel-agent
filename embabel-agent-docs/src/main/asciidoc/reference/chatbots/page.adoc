...
An `AssetStore` performs that copy and returns a `DurableAsset` containing portable metadata and an opaque storage URI:
...
When the wrapped tool returns a `MaterializableAsset`, the store copies its content before the durable result is added to the tracker.
The store must complete that copy before the original location expires.
...
NOTE: Embabel defines the storage contract but does not select a retention policy.
Applications and `AssetStore` implementations remain responsible for deciding when durable content can be deleted.
