...
[source,java]
----
// Create a session without context (fresh state)
ChatSession session = chatbot.createSession(user, outputChannel, null); // <1>

// Or create a session with a context to restore prior state
ChatSession resumedSession = chatbot.createSession(user, outputChannel, "user-workspace-123"); // <2>

session.onUserMessage(new UserMessage("What does this document say about taxes?")); // <3>
// Response is automatically sent to the outputChannel // <4>
----

<1> Create a new session for a user with an output channel (no context - fresh blackboard)
<2> Create a session that restores state from the "user-workspace-123" context
<3> Send a user message - triggers the agent to select and run an action
<4> Responses from actions are sent to the `outputChannel`
