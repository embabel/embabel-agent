...
[source,kotlin]
----
val runner = ai.withLlm("qwen3:latest")
               .withToolObject(Tooling::class.java)

val prompt = "What are exactly two the most hottest months in Florida and their respective highest temperatures"

// Use Kotlin extension function for streaming
val results: Flux<StreamingEvent<MonthItem>> = runner
    .withStreaming()
    .withPrompt(prompt)
    .createObjectStreamWithThinking<MonthItem>()

// Subscribe with real reactive callbacks
results
    .timeout(Duration.ofSeconds(150))
    .doOnSubscribe {
        logger.info("Stream subscription started")
...
val prompt = "What is the highest building in Paris?"

// Use Kotlin extension function for streaming
val results: Flux<String> = runner
    .withStreaming()
    .withPrompt(prompt)
    .generateStream()
