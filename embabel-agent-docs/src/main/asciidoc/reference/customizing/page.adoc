...
val embeddings: EmbeddingService = OpenAiCompatibleModelFactory.byokEmbedding(
    baseUrl = "https://api.acme.example.com/v1",
    apiKey = userKey,
...
provider and come back as an opaque authentication error. On success the returned
`EmbeddingService` already knows its dimension — it was observed during the probe — so no live `dimensions()` call is made later.
