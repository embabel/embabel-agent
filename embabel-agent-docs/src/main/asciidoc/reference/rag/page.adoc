...
The `PropertyFilter` sealed class hierarchy provides type-safe filter expressions:
...
====== Metadata vs Property Filters

`ToolishRag` accepts two separate filter parameters:

- **`metadataFilter`**: Filters on the `metadata` map of `Datum` objects. Metadata is typically ingestion-time information like source URI, ingestion date, owner ID, etc.
- **`propertyFilter`**: Filters on object properties. For `NamedEntityData`, this filters on the `properties` map. For typed entities, reflection is used to access top-level fields.
...
// Filter on entity properties (e.g., product category)
PropertyFilter propertyFilter = PropertyFilter.eq("category", "electronics");
...
In most cases, you'll use metadata filters for access control and property filters for business logic filtering.
...
- **Spring AI VectorStore**: Metadata filters are translated to `Filter.Expression` for native filtering; property filters are applied in-memory
- **Neo4j (Drivine)**: Both metadata and property filters are translated to native Cypher WHERE clauses (optimal performance)
...
return InMemoryPropertyFilter.filterResults(results, metadataFilter, propertyFilter);
----
