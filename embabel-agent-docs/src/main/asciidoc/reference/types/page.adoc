...
Common supported document formats include PDF, CSV, Microsoft Office documents (`.doc`, `.docx`, `.xlsx`), and OpenDocument files (`.odt`, `.ods`, `.odp`).

For uncommon document formats or if auto-detection fails, use `AgentDocument.create()` with an explicit MIME type.
