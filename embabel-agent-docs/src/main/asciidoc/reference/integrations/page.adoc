...
|`true`
|Tool loop execution (`embabel.tool_loop` + `embabel.tool_loop.completed`)

|`embabel.observability.trace-llm-calls`
|`true`
|LLM invocations with token usage (`embabel.llm.invocation`)

|`embabel.observability.trace-embedding`
|`true`
|Embedding invocations with token usage (`embabel.embedding`)
