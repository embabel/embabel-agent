 ...
 */
@Experimental
public final class TypeSafeClients {
    private TypeSafeClients() {}
    ...
    public static TypeSafeClient create(
            TypeSafeClientOptions options, Supplier<String> keySupplier) {
        return create(options, keySupplier, null, ObservationRegistry.NOOP);
    }

    /**
     * Creates a client from a cloned builder, without TypeSafe-owned observation handlers.
     *
     ...
     * @throws NullPointerException if options or keySupplier is null
     * @see #create(TypeSafeClientOptions, Supplier, RestClient.Builder, ObservationRegistry)
     */
    public static TypeSafeClient create(
    ...
    /**
     * Creates a client with one logical TypeSafe observation per API invocation.
     *
     ...
     * TypeSafe-owned observations guarantee fixed operation, outcome and status-family tags.
     *
     ...
     * Response limits apply to TypeSafe decoding; application buffering can occur before that
     * boundary. Credentials and transport/decoding failures are sanitized at the API boundary. A
     * missing or blank credential fails when the request runs, without exposing the supplier's
     * message.
     *
     * @param options validated non-secret settings
     ...
     * @param registry registry for logical TypeSafe observations and fallback HTTP observations;
     *     use ObservationRegistry.NOOP to disable those observations
    ...
    public static TypeSafeClient create(
        ...
        return new TypeSafeClient(
                new GuardedTypeSafeApi(options, keySupplier, builder, registry),
                options.model(),
                RetryPolicy.noRetry(),
                null);
    }
