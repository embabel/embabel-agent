...
class TerminalServices(
    ...
    fun handleProcessWaitingException(processWaitingException: ProcessWaitingException): AwaitableResponse? {
        val awaitable = processWaitingException.awaitable
        return when (awaitable) {
                ...
                when (control) {
                    is TextField -> {
                        ...
                        while (!isValid) {
                            ...
                            // Validate max length - fix smart cast issue
                            val maxLen = control.maxLength
                            if (maxLen != null && input.length > maxLen) {
                                lineReader.printAbove("Input exceeds maximum length of $maxLen characters.")
                                continue
                            }

                            // Validate pattern if specified - fix smart cast issue
                            val pattern = control.validationPattern
                            if (pattern != null && input.isNotBlank()) {
