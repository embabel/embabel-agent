...
class GoapPathToCompletionValidator : PathToCompletionAgentValidator {
    ...
    override fun validate(agentScope: AgentScope): ValidationResult {
        ...
        val ownHasRun = { action: Action -> Rerun.hasRunCondition(action) }
        ...
        // Self-rerun prevention hasRun_ conditions (key == ownHasRun) are excluded
        ...
        agentScope.actions.forEach { action ->
            val effectivePreconditions =
                action.preconditions.filterKeys { it != ownHasRun(action) }.keys
        ...
        agentScope.actions.forEach { action ->
            allConditions.addAll(action.preconditions.keys.filter { it != ownHasRun(action) })
        ...
        val goapActions = agentScope.actions.map { action ->
            ConditionAction(
                name = action.name,
                preconditions = action.preconditions.filterKeys { it != ownHasRun(action) },
