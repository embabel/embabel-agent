...
class SpringAiSingleLlmCaller(
    ...
    override fun call(
        ...
        // Convert Embabel tools to Spring AI tool callbacks
        val toolCallbacks = tools.map { it.toSpringAiToolCallback() }
...
/**
 * Adapter to convert an Embabel [Tool] to Spring AI's [ToolCallback].
 */
private fun Tool.toSpringAiToolCallback(): ToolCallback {
    val embabelTool = this
    return object : ToolCallback {
        override fun getToolDefinition(): ToolDefinition = ToolDefinition.builder()
            .name(embabelTool.definition.name)
            .description(embabelTool.definition.description)
            .inputSchema(embabelTool.definition.inputSchema.toJsonSchema())
            .build()

        override fun call(toolInput: String): String {
            val result = embabelTool.call(toolInput)
            return when (result) {
