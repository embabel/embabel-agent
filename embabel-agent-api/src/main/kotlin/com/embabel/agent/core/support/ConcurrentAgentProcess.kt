...
open class ConcurrentAgentProcess(
    ...
    override fun formulateAndExecutePlan(worldState: WorldState): AgentProcess {
        ...
        if (plan.isComplete()) {
            ...
            val process = this
            callbacks.forEach { it.beforeActionCoroutineLaunched(process) }
            val elapsed =
                measureTime {
                        ...
                        actions
                            .map { action ->
                                CoroutineScope(Dispatchers.IO).async {
                                    try {
                                        callbacks.forEach { it.onActionCoroutineLaunched(process, action) }
                                        executeAction(action)
                                    } finally {
                                        callbacks.forEach { it.onActionCoroutineCompleted(process, action) }
                                    }
                                }
                            }.map { deferred ->
                                runBlocking {
