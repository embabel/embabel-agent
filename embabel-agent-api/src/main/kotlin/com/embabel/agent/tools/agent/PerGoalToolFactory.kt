...
class PerGoalToolFactory(
    ...
    /**
     * Build readable discriminator candidates for a goal's starting input types. Types sharing a
     * simple name fall back to a sanitized qualified name; this normalization can still collide,
     * so uniqueness must be checked on the complete published tool list.
     */
    private fun discriminatorNames(inputTypes: Set<Class<*>>): Map<Class<*>, String> {
        ...
        return inputTypes.associateWith { type ->
            if (simpleNameCounts[type.simpleName] == 1 && type.simpleName.isNotBlank()) {
                ...
                type.name.replace(Regex("[^a-zA-Z0-9_-]"), "_")
            }
        }
    }
