 ...
 */
package com.embabel.agent.config.models.openai.compatible
...
/**
 * Configuration properties for OpenAI Compatible model settings.
 * These properties can be set in application.properties/yaml using the
 * prefix embabel.agent.platform.models.openai.compatible
 */
@ConfigurationProperties(prefix = "embabel.agent.platform.models.openai.compatible")
class OpenAiCompatibleProperties : RetryProperties {
    /**
 ...
 * OPENAI_COMPATIBLE_CUSTOM_MODELS=llama-3.3-70b-versatile,mixtral-8x7b-32768,gemma2-9b-it
...
@ExcludeFromJacocoGeneratedReport(reason = "OpenAi Compatible configuration can't be unit tested")
class OpenAiCompatibleModelsConfig(
    @Value("\${OPENAI_COMPATIBLE_BASE_URL:#{null}}")
    baseUrl: String?,
    @Value("\${OPENAI_COMPATIBLE_API_KEY}")
    apiKey: String,
    @Value("\${OPENAI_COMPATIBLE_CUSTOM_MODELS:#{null}}")
    ...
    init {
        logger.info("OpenAI Compatible models are available: {}", properties)
        if (customModelList.isNotEmpty()) {
            logger.info("Custom OpenAI-compatible models configured: {}", customModelList)
        }
    }

    @Bean
    fun openAiCompatibleModelsInitializer(): ProviderInitialization {
    ...
    companion object {
        private const val CUSTOM_PROVIDER = "OpenAI-Compatible"
    }
}
