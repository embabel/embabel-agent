 ...
 */
package com.embabel.agent.jev.example;
...
class JevConsumerTest {
    @Test void plainSpringConsumerUsesAllThreeNativePrimitives() {
        ...
        new ApplicationContextRunner().withUserConfiguration(ConsumerConfiguration.class)
                ...
                .withPropertyValues("embabel.agent.platform.models.jev.enabled=true", "embabel.agent.platform.models.jev.api-key=test-key")
                .run(context -> {
                    assertThat(context).hasNotFailed();
                    var response = context.getBean(TriageSignals.class).evaluate("My payment failed");
    ...
    public record TriageSignals(TypeSafeClient client) {
        public SystemOneResponse evaluate(String message) {
            return client.systemOne(Map.of("message", message), Map.of(
                    "urgent", Noul.of("Does the message require urgent attention?"),
                    "team", Choice.of("Which team should handle the message?", "billing", "support"),
    ...
    static class ConsumerConfiguration {
        @Bean TriageSignals triageSignals(TypeSafeClient client) { return new TriageSignals(client); }
    }
}
